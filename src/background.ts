...
chrome.alarms.onAlarm.addListener((alarm) => {
  ...
  if (alarm.name.startsWith('whatsCrm-')) {
    ...
    chrome.storage.local.get([SCHEDULED_MESSAGES_KEY], (result) => {
      let scheduledMessages: ScheduledMessage[] = (result[SCHEDULED_MESSAGES_KEY] as ScheduledMessage[]) || [];
      let messageToSend: ScheduledMessage | null = null;
      let messageIndex = -1;
          ...
          if (tabId) {
            chrome.tabs.sendMessage(
              tabId,
              {
                action: "sendMessage",
                payload: {
                  contact: (messageToSend as ScheduledMessage).contact,
                  message: (messageToSend as ScheduledMessage).template,
                },
              },
              (response) => {
